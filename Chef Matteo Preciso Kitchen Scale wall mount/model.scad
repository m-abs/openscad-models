    ...
    diff()
    {
        rect_tube(h = inner_length, isize = [ inner_width, inner_height ], wall = wall_thickness, anchor = BOTTOM)
        {
            ...
            position(BACK + BOTTOM) fwd(wall_thickness)
            {
                ...
                {
                    cuboid([ inner_width, display_support_height, display_support_length ], chamfer = wall_thickness,
                           anchor = BACK + BOTTOM, edges = TOP + FRONT);
                }

                cuboid([ display_support_length, display_support_height, display_support_offset ],
                       anchor = BACK + BOTTOM);
            }
