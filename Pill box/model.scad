...
module make() {
  ...
  module lid_base_shape(anchor = BOTTOM + CENTER) {
    cuboid(
      [lid_width1, depth + 0.001, lid_height], anchor=anchor, rounding=lid_height / 2,
  ...
  module lid(text) {
    diff() {
      lid_base_shape() {
        position(TOP) up(0.5) text3d(text=text, h=1, anchor=CENTER, center=true);
        tag("remove") position(BACK + TOP) fwd(1) cyl(r=0.5, l=lid_width2, $fn=8, orient=LEFT);
        tag("remove") position(FRONT + TOP) back(1) cyl(r=0.5, l=lid_width2, $fn=8, orient=LEFT);
      }
    }
  }
